...
python scripts/install_sat3d.py \
  --root <workspace>/SAT3D \
  --base-python <conda-env>/bin/python
```
...
```bash
export SAT3D_ROOT=<workspace>/SAT3D
export SAT3D_RUNTIME_PYTHON=<workspace>/SAT3D/.venv/bin/python
export SAT3D_MODEL_CHECKPOINT=<workspace>/SAT3D/weights/sam_model_dice_best.pth
export SAT3D_CRITIC_CHECKPOINT=<workspace>/SAT3D/weights/critic_dice_best.pth
