...
**问题:** `_validate_path()` 使用 `startswith("/")` 拒绝所有绝对路径。但 `ct_path` 始终是绝对路径（如 `/home/user/ct.nii.gz`），导致术前规划 API 始终返回 400 错误。
...
- [x] `_validate_path` 现在接受绝对路径（如 `/home/user/ct.nii.gz`）
