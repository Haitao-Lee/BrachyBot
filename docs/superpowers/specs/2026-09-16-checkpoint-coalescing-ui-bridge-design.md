...
- 在现有 `with self._lock:` 内先判断：若 `key` 在 `_checkpoint_inflight` → `_checkpoint_dirty[key] = True`，`logger.debug("workspace checkpoint coalesced ...")`，**直接返回**（不 bump generation、不动定时器）。
- 否则维持现状。
