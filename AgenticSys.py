...
class BrachyAgent:
    ...
    def _run_llm_function_calling(self, message: str, steps: List[Dict], step_id_ref: List[int]) -> str:
        ...
        system_prompt = (
            ...
            "\n"
            "  **IMPORTANT: Search Query Language**\n"
            "  - Search engines work best with English queries\n"
            "  - When searching, use English keywords (translate if needed)\n"
            "  - Example: '前列腺癌剂量' -> search 'prostate cancer dose'\n"
            "  - Example: '你知道DeepRare吗' -> search 'DeepRare'\n"
            "  - Final response MUST match user's language (Chinese in -> Chinese out)\n"
            "  - Internal processing can be in English for better search results\n"
            "\n"
