...
class BrachyAgent:
    ...
    def _run_llm_function_calling(self, message: str, steps: List[Dict], step_id_ref: List[int]) -> str:
        ...
        system_prompt = (
            ...
            "\n"
            "  **IMPORTANT: Search Query Translation**\n"
            "  - If user asks in Chinese, translate the query to English before searching\n"
            "  - Example: '前列腺癌剂量' -> search for 'prostate cancer dose'\n"
            "  - Example: '你知道DeepRare吗' -> search for 'DeepRare'\n"
            "  - Keep medical terms in English (PubMed is English-only)\n"
            "  - You can call the LLM multiple times - first translate, then search, then present\n"
            "\n"
