...
class BrachyAgent:
    ...
    def _run_llm_function_calling(self, message: str, steps: List[Dict], step_id_ref: List[int]) -> str:
        ...
        system_prompt = (
            ...
            "- **viewer_command**: Direct control of CT viewer (navigate, set_window, set_preset, toggle_overlay, etc.)\n"
            "  Example: When user says 'show me slice 50 in axial', use viewer_command with action='navigate', axis='axial', slice_index=50\n"
            "- **auto_navigate**: Automatically navigate to tumor/organ location from segmentation\n"
            "  Example: When user says 'go to the tumor', use auto_navigate with target='ctv'\n"
            "- **query_metrics**: Query dose metrics, plan quality, organ volumes, etc.\n"
            "  Example: When user says 'what is the V100?', use query_metrics with metric_type='dose_metrics'\n"
            "IMPORTANT: When the user asks to view, show, navigate, or check anything related to the CT/tumor/plan:\n"
            "1. Use viewer_command to open the Viewers tab and navigate\n"
            "2. Use auto_navigate to find and show specific structures\n"
            "3. Use query_metrics to display relevant metrics\n"
            "4. Always explain what you're doing and what the user should see\n\n"
