#!/usr/bin/env python3
"""
Agent 2 Benchmark Runner - Memory-efficient version
Reuses a single Playwright browser instance for all screenshots
instead of launching a new browser per test case.
"""
...
BENCHMARK_DIR = "<workspace>/BrachyBot/benchmarks"
...
class BrowserManager:
    """Manages a single Playwright browser for all screenshots."""
    def __init__(self):
        self.pw = None
        self.browser = None
        self.page = None
    ...
    def screenshot(self, case_id, cat_num):
...
def run_category(cat_num, agent_id, browser_mgr):
    ...
    for i, test_case in enumerate(test_cases):
        ...
        screenshot_path = browser_mgr.screenshot(case_id, cat_num)
...
if __name__ == "__main__":
    ...
    if not categories:
        print(f"Usage: python agent2_runner.py <agent_id> <category1> <category2> ...")
        sys.exit(1)

    print(f"Agent {agent_id} Benchmark Runner (Memory-Efficient)")
    print(f"Categories: {categories}")
    print(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    # Start a single browser instance for all screenshots
    browser_mgr = BrowserManager()
    browser_started = browser_mgr.start()

    all_results = []
    for cat_num in categories:
        try:
            results = run_category(cat_num, agent_id, browser_mgr)
    ...
    if incomplete:
        ...
        for cat in incomplete[:3]:
            ...
            if cat['category'] not in categories:
                ...
                try:
                    help_results = run_category(cat['category'], agent_id, browser_mgr)
                    all_results.extend(help_results)
                except Exception as e:
                    print(f"  Category {cat['category']} ERROR: {e}")
