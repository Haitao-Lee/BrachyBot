#!/usr/bin/env python3
"""
Benchmark Agent 2 Runner v3 - Categories 10-18
Maximum robustness: retries, timeouts, intermediate saves, signal handling.
"""
import json, os, sys, time, glob, requests, signal
from datetime import datetime
from pathlib import Path

BASE_URL = "http://localhost:8080"
SCREENSHOT_DIR = "<workspace>/BrachyBot/docs/benchmark_result/screenshots"
REPORT_DIR = "<workspace>/BrachyBot/docs/benchmark_result/reports"
AGENT_ID = 2
RESULTS_FILE = f"{REPORT_DIR}/agent{AGENT_ID}_results_v3.json"
RUN_LOG = f"<workspace>/BrachyBot/docs/benchmark_result/agent2_v3_run.log"
...
def handle_signal(signum, frame):
    ...
    print(f"\nShutdown requested (signal {signum}). Finishing current test...")
    sys.stdout.flush()
...
def send_message(text, session_id, timeout=120, max_retries=2):
    payload = {
        "message": text,
        "clear_context": True,
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                log(f"    Error on attempt {attempt+1}: {e}, retrying...")
                time.sleep(2)
            else:
...
def run_category(cat_num):
    files = glob.glob(f"<workspace>/BrachyBot/benchmarks/{cat_num:02d}_*.json")
    if not files:
        log(f"No benchmark file found for category {cat_num}")
        return []
    ...
    log(f"\n{'='*60}")
    log(f"Category {cat_num}: {cat_name} ({len(test_cases)} cases)")
    log(f"{'='*60}")

    results = []
    for i, test_case in enumerate(test_cases):
        if shutdown_requested:
            log("Shutdown requested, stopping category...")
            break

        case_id = test_case.get('id', f'Q{i+1:04d}')
        input_text = test_case.get('input', '')

        log(f"  [{i+1}/{len(test_cases)}] {case_id}...", )

        session_id = f"agent{AGENT_ID}_{cat_num:02d}_{case_id}_{int(time.time() * 1000)}"

        start_time = time.time()
        response = send_message(input_text, session_id, timeout=120, max_retries=2)
        ...
        log(f"    {status} ({total_score:.2f}) [{response_time:.1f}s]")
        if root_cause:
            log(f"    -> {root_cause}: {root_cause_detail}")

    return results

def take_screenshots_batch(all_results):
    """Take screenshots using a single Playwright browser session."""
    log(f"\n{'='*60}")
    log(f"Phase 2: Taking screenshots for {len(all_results)} cases")
    log(f"{'='*60}")
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            for i, r in enumerate(all_results):
                ...
                try:
                    log(f"  [{i+1}/{len(all_results)}] Screenshot {case_id}...")
        ...
        log(f"\nPlaywright error: {e}")
        log("Continuing without screenshots...")
...
def generate_report(all_results):
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        if root_causes:
            ...
            f.write("|------------|-------|---------------|----------|-------------|\n")
            severity_map = {
                'hallucination': 'P0', 'safety_leak': 'P0',
                'keyword_missing': 'P2', 'wrong_answer': 'P2',
                'too_brief': 'P2', 'too_verbose': 'P2',
                'context_lost': 'P1', 'tool_misfire': 'P1',
...
if __name__ == "__main__":
    ...
    with open(RUN_LOG, 'w') as f:
        f.write(f"Benchmark Agent {AGENT_ID} v3\n")
        f.write(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")

    log(f"Benchmark Agent {AGENT_ID} v3 - Maximum Robustness")
    log(f"Categories: {CATEGORIES}")
    log(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    log(f"Phase 1: API tests")
    log(f"Phase 2: Batch screenshots")
    ...
    try:
        for cat_num in CATEGORIES:
            ...
            log(f"  [Intermediate save: {len(all_results)} results]")
    ...
    if all_results:
        try:
            all_results = take_screenshots_batch(all_results)
        except Exception as e:
            log(f"Screenshot phase error: {e}")
        ...
        log(f"\n{'='*60}")
        log(f"Agent {AGENT_ID} Complete!")
        log(f"Total: {len(all_results)} tests, {sum(1 for r in all_results if r['passed'])} passed")
