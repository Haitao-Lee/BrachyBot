...
SCRIPT="<workspace>/BrachyBot/benchmarks/agent1_mini.py"
RESULTS="<workspace>/BrachyBot/docs/benchmark_result/reports/agent1_all_results.json"

# Initialize if needed
if [ ! -s "$RESULTS" ]; then echo "[]" > "$RESULTS"; fi

for cat in 1 2 3 4 5 6 7 8 9; do
    FILE=$(ls <workspace>/BrachyBot/benchmarks/$(printf '%02d' $cat)_*.json 2>/dev/null | head -1)
...
import glob, os
files=glob.glob('<workspace>/BrachyBot/benchmarks/$(printf '%02d' $cat)_*.json')
