#!/usr/bin/env python3
"""
Agent 4 Direct Runner - Runs remaining benchmark tests for assigned categories.
Uses longer timeouts, waits for server stability, takes screenshots.
"""
        ...
        try:
            if not check_server():
                ...
                if not wait_for_server(120):
                    print("    Server did not come back", flush=True)
            ...
            print(f"    Error attempt {attempt+1}/3: {type(e).__name__}: {str(e)[:100]}", flush=True)
            if attempt < 2:
                time.sleep(5)
...
def main():
    categories = [int(x) for x in sys.argv[1:]] if len(sys.argv) > 1 else [23, 19, 22, 21, 18, 9, 15, 7]

    print("=" * 60)
    print(f"AGENT {AGENT_ID} DIRECT RUNNER")
    print("=" * 60)
    ...
    # Save results
    results_file = f"{REPORT_DIR}/agent4_results_{int(time.time())}.json"
