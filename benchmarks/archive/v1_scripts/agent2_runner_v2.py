#!/usr/bin/env python3
"""
Benchmark Agent 2 Runner v2 - Categories 10-18
Phase 1: Run ALL API tests and score (fast, no screenshots)
Phase 2: Batch screenshots using single Playwright session
NO CODE FIXES to application code.
"""
...
AGENT_ID = 2
RESULTS_FILE = f"{REPORT_DIR}/agent{AGENT_ID}_results_v2.json"
...
def run_category(cat_num):
    """Run all test cases for a category via API."""
...
def take_screenshots_batch(all_results):
    """Take screenshots for all cases using a single Playwright session."""
    print(f"\n{'='*60}")
    print(f"Phase 2: Taking screenshots for {len(all_results)} cases")
    print(f"{'='*60}")
    sys.stdout.flush()
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            for i, r in enumerate(all_results):
                ...
                try:
                    print(f"  [{i+1}/{len(all_results)}] Screenshot {case_id}...", end=" ", flush=True)
...
if __name__ == "__main__":
    ...
    print(f"Benchmark Agent {AGENT_ID} v2")
    print(f"Categories: {CATEGORIES}")
    print(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"Phase 1: API tests (no screenshots)")
    print(f"Phase 2: Batch screenshots")
    sys.stdout.flush()
    ...
    if all_results:
        ...
        try:
            ...
            print(f"Screenshot phase failed: {e}")
