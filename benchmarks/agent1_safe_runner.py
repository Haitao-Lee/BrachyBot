#!/usr/bin/env python3
"""Agent 1 - Safe runner. Saves to agent1/ subdirectory to avoid conflicts."""
import json, os, sys, time, glob, requests
from datetime import datetime

BASE_URL = "http://localhost:8080"
BENCHMARK_DIR = "<workspace>/BrachyBot/benchmarks"
RESULTS_DIR = "<workspace>/BrachyBot/docs/benchmark_result/agent1"
os.makedirs(RESULTS_DIR, exist_ok=True)

def load_results(cat_num):
    f = f"{RESULTS_DIR}/cat{cat_num:02d}.json"
    try:
        with open(f) as fp:
            return json.load(fp)
    except:
...
if __name__ == "__main__":
    ...
    for cat_num in categories:
        ...
        if not remaining:
            print(f"\nCat {cat_num}: {cat_name} - ALL DONE ({len(existing)}/{len(cases)})")
            sys.stdout.flush()
            continue
    ...
    for cat_num in categories:
        try:
            with open(f"{RESULTS_DIR}/cat{cat_num:02d}.json") as fp:
                d = json.load(fp)
            valid = [r for r in d if r.get('response_length', 0) > 0]
            p = sum(1 for r in valid if r['passed'])
            total_all += len(valid)
            passed_all += p
            print(f"  Cat {cat_num:02d}: {p}/{len(valid)} passed")
        except:
            print(f"  Cat {cat_num:02d}: MISSING")
