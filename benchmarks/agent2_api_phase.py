#!/usr/bin/env python3
"""Phase 1: API tests only - no screenshots, minimal memory."""
import json, os, sys, time, glob, requests
from datetime import datetime

BASE_URL = 'http://localhost:8080'
RESULTS_FILE = '<workspace>/BrachyBot/docs/benchmark_result/reports/agent2_final_results.json'
AGENT_ID = 2
...
def analyze_failure(response, test_case):
    for kw in ["I don't know", "I'm not sure", 'I cannot verify', "I'm just guessing"]:
        if kw.lower() in response.lower():
            return 'hallucination', 'Contains: ' + kw
    for kw in test_case.get('forbidden_keywords', []):
        if kw.lower() in response.lower():
            return 'safety_leak', 'Forbidden: ' + kw
...
if os.path.exists(RESULTS_FILE):
    try:
        ...
        for r in all_results:
            ...
            completed.add('%d_%s' % (r['category_num'], r['case_id']))
        # Rebuild all_results keeping only valid
        all_results = [r for r in all_results if not r.get('response', '').startswith('Error:')]
        print('Loaded %d valid results, %d completed keys' % (len(all_results), len(completed)))
    except Exception as e:
        print('Load error: %s' % e)

CATEGORIES = [10, 11, 12, 13, 14, 15, 16, 17, 18]
print('Start: %s' % datetime.now().strftime('%H:%M:%S'))
sys.stdout.flush()

for cat_num in CATEGORIES:
    files = glob.glob('<workspace>/BrachyBot/benchmarks/%02d_*.json' % cat_num)
    ...
    print('\nCat %d: %s (%d total, %d remaining)' % (cat_num, cat_name, len(cases), remaining))
    sys.stdout.flush()
    ...
    with open(RESULTS_FILE, 'w') as f: json.dump(all_results, f, indent=2, default=str)
    print('  [Saved %d total]' % len(all_results))
    sys.stdout.flush()

total = len(all_results)
passed_n = sum(1 for r in all_results if r['passed'])
print('\nDone: %d tests, %d passed (%.1f%%)' % (total, passed_n, passed_n/total*100 if total else 0))
print('End: %s' % datetime.now().strftime('%H:%M:%S'))
