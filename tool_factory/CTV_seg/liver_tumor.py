...
============================
Segments liver tumors from CT images using TotalSegmentator.
"""
...
class LiverTumorSegmentationTool(BaseTool):
    """
    Tool for segmenting liver tumors from CT images using TotalSegmentator.

    Uses TotalSegmentator's liver_vessels task which produces:
    - Label 1: liver
    - Label 2: liver vessels (including tumors)
    This tool extracts label 2 (tumor) for CTV.
    """
    ...
    def description(self) -> str:
        return (
            "Segment liver tumors from CT images using TotalSegmentator liver_vessels task. "
            "Extracts tumor label (label 2) from the segmentation output. "
    ...
    def _execute(self, **kwargs) -> ToolResult:
        ...
        return ToolResult(
            success=True,
            data=ctv_mask,
            message=f"Liver tumor segmentation completed using {method}. Found {ctv_voxel_count} tumor voxels ({ctv_volume_mm3:.1f} mm³).",
            metadata={
                "ctv_mask": ctv_mask,
                "ctv_array": ctv_array,
    ...
    def _totalsegmentator_segmentation(self, image: sitk.Image, target_value: float, fast_mode: bool):
        ...
        try:
            ...
            result_file = os.path.join(output_dir, "liver_vessels.nii.gz")
