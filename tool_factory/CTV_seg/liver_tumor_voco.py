...
VoCo Liver Tumor Segmentation Tool
====================================
Liver tumor segmentation using VoCo pre-trained SwinUNETR.
Fine-tuned on 3D-IRCADb dataset.
"""
...
class VoCoLiverTumorTool(BaseTool):
    """
    VoCo-based liver tumor segmentation tool.

    Uses SwinUNETR fine-tuned on 3D-IRCADb dataset.
    """

    MODEL_PATH = os.environ.get("VOCO_MODEL_PATH", os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", "VoCo", "3D-IRCADb", "model_voco_74.27.pt"))
    OUT_CHANNELS = 3
    FEATURE_SIZE = 48
    ROI_SIZE = (96, 96, 96)
    SPACING = (1.5, 1.5, 1.5)
    ...
    @property
    def name(self) -> str:
        return "voco_liver_tumor"

    @property
    def description(self) -> str:
        return (
            "Segment liver tumors from CT images using VoCo pre-trained SwinUNETR. "
            "Fine-tuned on 3D-IRCADb dataset (DSC: 74.27%). "
    ...
    def _get_inverse_transforms(self, original_shape):
        from monai.transforms import Compose, Invertd, Spacingd, Orientationd, CropForegroundd, SpatialPadd
        # These inverse transforms map prediction back to original space
        return None  # We'll handle inverse manually
    ...
    def _execute(self, **kwargs) -> ToolResult:
        ...
        return ToolResult(
            success=True,
            data=tumor_array,
            message=f"Liver tumor segmentation completed. Volume: {volume_mm3:.1f} mm³",
            metadata={
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="VoCo Liver Tumor Segmentation")
