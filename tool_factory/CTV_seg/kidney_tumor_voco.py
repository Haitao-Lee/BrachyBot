"""
VoCo Kidney Tumor Segmentation Tool
=================================
Kidney tumor segmentation using VoCo pre-trained SwinUNETR.
Fine-tuned on KiPA22 dataset.
"""
...
class VoCoKidneyTumorTool(BaseTool):
    """
    VoCo-based kidney tumor segmentation tool.

    Uses SwinUNETR fine-tuned on KiPA22 dataset.
    """

    MODEL_PATH = os.environ.get("VOCO_MODEL_PATH", "/home/lht/snap/brachyplan/BrachyBot/VoCo/Kipa/model_voco.pt")))), "VoCo", os.path.basename(__file__).replace("_voco.py", "").replace("_tumor", "").replace("_vessel", "").upper() + "/model_voco.pt"))
    OUT_CHANNELS = 2
    FEATURE_SIZE = 48
    ROI_SIZE = (96, 96, 96)
    SPACING = (1.5, 1.5, 1.5)
    ...
    @property
    def name(self) -> str:
        return "voco_kidney_tumor"

    @property
    def description(self) -> str:
        return (
            "Segment kidney tumors from CT images using VoCo pre-trained SwinUNETR. "
            "Fine-tuned on KiPA22 dataset. "
    ...
    def _execute(self, **kwargs) -> ToolResult:
        ...
        return ToolResult(
            success=True,
            data=tumor_array,
            message=f"Kidney tumor segmentation completed. Volume: {volume_mm3:.1f} mm³",
            metadata={
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="VoCo Kidney Tumor Segmentation")
