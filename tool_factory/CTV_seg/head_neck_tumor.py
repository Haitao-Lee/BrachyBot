"""
Head and Neck Tumor Segmentation Tool
====================================
Segments head and neck tumors from CT images using TotalSegmentator.
"""
...
class HeadNeckTumorSegmentationTool(BaseTool):
    """
    Tool for segmenting head and neck tumors from CT images.

    Uses TotalSegmentator's head and neck task for segmentation.
    ...
    def description(self) -> str:
        return (
            "Segment head and neck tumors from CT images using TotalSegmentator. "
            "Uses the total task which includes head and neck structures. "
    ...
    def _execute(self, **kwargs) -> ToolResult:
        ...
        return ToolResult(
            success=True,
            data=ctv_mask,
            message=f"Head and neck tumor segmentation completed using {method}. Found {ctv_voxel_count} tumor voxels ({ctv_volume_mm3:.1f} mm³).",
            metadata={
                "ctv_mask": ctv_mask,
                "ctv_array": ctv_array,
    ...
    def _totalsegmentator_segmentation(self, image: sitk.Image, target_value: float, fast_mode: bool):
        ...
        try:
            ...
            logger.info(f"Running TotalSegmentator head_neck: {' '.join(cmd)}")
            ...
            if not os.path.exists(result_file):
                raise RuntimeError(f"TotalSegmentator output not found")
