...
class VoCoProstateTool(BaseTool):
    ...
    MODEL_PATH = os.environ.get("VOCO_MODEL_PATH", "<workspace>/BrachyBot/VoCo/Amos-MR/model_voco_79.24.pt")))), "VoCo", os.path.basename(__file__).replace("_voco.py", "").replace("_tumor", "").replace("_vessel", "").upper() + "/model_voco.pt"))
    OUT_CHANNELS = 2
    FEATURE_SIZE = 48
    ROI_SIZE = (96, 96, 96)
    SPACING = (1.0, 1.0, 1.0)
