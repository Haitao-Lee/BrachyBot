"""
VoCo Lung Tumor Segmentation Tool
================================
Lung tumor segmentation using VoCo pre-trained SwinUNETR.
Fine-tuned on MSD Task06 Lung dataset.
"""
...
class VoCoLungTumorTool(BaseTool):
    """
    VoCo-based lung tumor segmentation tool.

    Uses SwinUNETR fine-tuned on MSD Task06 Lung dataset (DSC: 75.74%).
    """

    MODEL_PATH = os.environ.get("VOCO_MODEL_PATH", "/home/lht/snap/brachyplan/BrachyBot/VoCo/Lung/model_voco_75.74.pt")))), "VoCo", os.path.basename(__file__).replace("_voco.py", "").replace("_tumor", "").replace("_vessel", "").upper() + "/model_voco.pt"))
    OUT_CHANNELS = 2
    FEATURE_SIZE = 48
    ROI_SIZE = (96, 96, 96)
    SPACING = (1.5, 1.5, 1.5)
    ...
    @property
    def name(self) -> str:
        return "voco_lung_tumor"

    @property
    def description(self) -> str:
        return (
            "Segment lung tumors from CT images using VoCo pre-trained SwinUNETR. "
            "Fine-tuned on MSD Task06 Lung dataset (DSC: 75.74%). "
    ...
    def _execute(self, **kwargs) -> ToolResult:
        ...
        return ToolResult(
            success=True,
            data=tumor_array,
            message=f"Lung tumor segmentation completed. Volume: {volume_mm3:.1f} mm³",
            metadata={
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="VoCo Lung Tumor Segmentation")
