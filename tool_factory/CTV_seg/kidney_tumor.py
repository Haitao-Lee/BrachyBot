"""
Kidney Tumor Segmentation Tool
==============================
Segments kidney tumors from CT images using TotalSegmentator or nnU-Net.
"""
...
class KidneyTumorSegmentationTool(BaseTool):
    """
    Tool for segmenting kidney tumors from CT images.

    Uses TotalSegmentator's kidney task for segmentation.
    The TotalSegmentator produces separate labels for left and right kidney.
    """
    ...
    def description(self) -> str:
        return (
            "Segment kidney tumors from CT images using TotalSegmentator. "
            "Uses the kidney task which produces kidney labels. "
            "Returns a binary mask where target_value indicates tumor/kidney tissue."
        )
    ...
    def _execute(self, **kwargs) -> ToolResult:
        ...
        return ToolResult(
            success=True,
            data=ctv_mask,
            message=f"Kidney tumor segmentation completed using {method}. Found {ctv_voxel_count} tumor voxels ({ctv_volume_mm3:.1f} mm³).",
            metadata={
                "ctv_mask": ctv_mask,
                "ctv_array": ctv_array,
    ...
    def _totalsegmentator_segmentation(self, image: sitk.Image, target_value: float, fast_mode: bool):
        ...
        try:
            ...
            logger.info(f"Running TotalSegmentator kidney: {' '.join(cmd)}")
            ...
            result_file = os.path.join(output_dir, "kidney.nii.gz")
            if not os.path.exists(result_file):
                result_file = os.path.join(output_dir, "segmentations.nii.gz")

            if not os.path.exists(result_file):
                raise RuntimeError(f"TotalSegmentator kidney output not found")
