...
Fine-tuned on BTCV (Beyond the Cranial Vault) dataset.
...
class VoCoBTCVTumorTool(BaseTool):
    """
    VoCo-based multi-organ tumor segmentation tool.

    Uses SwinUNETR fine-tuned on BTCV dataset (DSC: 86.64%).
    Suitable for liver, kidney, spleen, and pancreas tumors.
    """

    MODEL_PATH = os.environ.get("VOCO_MODEL_PATH", "<workspace>/BrachyBot/VoCo/BTCV/model_voco_86.64.pt")))), "VoCo", os.path.basename(__file__).replace("_voco.py", "").replace("_tumor", "").replace("_vessel", "").upper() + "/model_voco.pt"))
    OUT_CHANNELS = 2
    FEATURE_SIZE = 48
    ROI_SIZE = (96, 96, 96)
    SPACING = (1.5, 1.5, 1.5)
    ...
    @property
    def name(self) -> str:
        return "voco_btcv_tumor"

    @property
    def description(self) -> str:
        return (
            "Segment abdominal tumors from CT images using VoCo pre-trained SwinUNETR. "
            "Fine-tuned on BTCV dataset (DSC: 86.64%). "
            "Supports liver, kidney, spleen, and pancreas tumor segmentation. "
    ...
    def _execute(self, **kwargs) -> ToolResult:
        ...
        return ToolResult(
            success=True,
            data=tumor_array,
            message=f"BTCV tumor segmentation completed. Volume: {volume_mm3:.1f} mm³",
            metadata={
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="VoCo BTCV Tumor Segmentation")
