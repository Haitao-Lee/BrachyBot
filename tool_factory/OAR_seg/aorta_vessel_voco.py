...
class VoCoAortaVesselTool(BaseTool):
    """
    VoCo-based blood vessel segmentation tool.

    Uses SwinUNETR fine-tuned on Aorta dataset (DSC: 69.64%).
    ...
    @property
    def name(self) -> str:
        return "voco_aorta_vessel"

    @property
    def description(self) -> str:
        return (
            "Segment blood vessels (aorta and major vessels) from CT images using VoCo pre-trained SwinUNETR. "
            "Fine-tuned on Aorta dataset (DSC: 69.64%). "
            "Input: CT image (SimpleITK) or path. "
            "Output: Vessel segmentation mask and volume metrics."
        )
    ...
    def output_schema(self) -> Dict:
        return {
            "type": "object",
            "properties": {
                "vessel_mask": {"type": "object"},
                "vessel_array": {"type": "array"},
                "vessel_volume_mm3": {"type": "number"},
                "vessel_voxel_count": {"type": "integer"},
            },
        }
    ...
    def _execute(self, **kwargs) -> ToolResult:
        ...
        try:
            vessel_array = self._inference(image)
        except FileNotFoundError as e:
            return ToolResult(success=False, error=str(e))
        except Exception as e:
            return ToolResult(success=False, error=f"Inference failed: {str(e)}")

        vessel_array = (vessel_array == 1).astype(np.uint8)

        voxel_count = int(np.sum(vessel_array > 0))
        spacing = image.GetSpacing()
        volume_mm3 = voxel_count * spacing[0] * spacing[1] * spacing[2]

        vessel_mask = sitk.GetImageFromArray(vessel_array)
        vessel_mask.CopyInformation(image)

        return ToolResult(
            success=True,
            data=vessel_array,
            message=f"Aorta/vessel segmentation completed. Volume: {volume_mm3:.1f} mm³",
            metadata={
                "vessel_mask": vessel_mask,
                "vessel_array": vessel_array,
                "vessel_volume_mm3": float(volume_mm3),
                "vessel_voxel_count": voxel_count,
            },
        )
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="VoCo Aorta/Vessel Segmentation")
    parser.add_argument("--image", required=True, help="Input CT image path")
    parser.add_argument("--output", help="Output vessel mask path")
    ...
    if result.success and args.output:
        sitk.WriteImage(result.metadata["vessel_mask"], args.output)
