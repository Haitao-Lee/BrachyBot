...
class UIScreenshotTool(BaseTool):
    ...
    def description(self) -> str:
        return (
            ...
            "same assistant response. For `data-tree`, the browser temporarily "
            "opens the real Viewer page, expands the requested node's live ancestor "
            "groups, scrolls that actual Data Tree row into a readable crop, captures "
            "the real on-screen DOM, and then restores the user's panel, expansion, "
            "selection, width, and scroll state. Never synthesize a replacement Data "
            "Tree card. Set visual_purpose, "
            ...
            "verifiably visible; it never permits marking a hidden, unloaded, "
            "unavailable, or out-of-view object. Use target_refs for stable UI/Data Tree/scene "
            ...
            "annotation_policy=`required`. A generated object whose status is stale "
            "may still be located when its mesh/node is actually loaded and visible; "
            "stale means outdated, not absent. The answer must report those states "
            "separately. The browser computes the element "
