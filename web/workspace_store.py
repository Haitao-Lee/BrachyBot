...
DEFAULT_ARCHIVE_ROOT = os.environ.get("BRACHYBOT_ARCHIVE_ROOT") or str(
    Path(__file__).resolve().parents[1] / "runtime" / "archive"
)
        ...
        *,
        ...
        try:
            ...
            with self._lock:
                ...
                if (
                    pending is not None
                    and int(self._checkpoint_generations.get(key, 0)) == generation_at_entry
                ):
                    ...
                    logger.debug(
                        "workspace checkpoint coalesced payload superseded session=%s reason=%s",
                        session_id, reason,
                    )
