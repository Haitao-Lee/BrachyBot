...
def create_app(config: Optional[Dict] = None):
    ...
    def _build_report_interpretation(agent, language="zh"):
        ...
        lines = [
            "**剂量学评估与临床解读**" if zh else "**Dosimetric Evaluation & Clinical Interpretation**",
            "",
            (
                "本节仅报告观测到的计划指标。临床通过/不通过结论必须引用适用的部位特异性指南，"
                "或使用已确认的病例方案限值。"
                if zh else
            ...
            "",
            "**观测指标**" if zh else "**Observed metrics**",
        ]
        ...
        if score is not None:
            try:
                lines.append(
                    f"- 内部计划评分：{float(score):.0f}/100。该分数仅用于质量排序提示，不代表临床批准。"
                    if zh else
                    f"- Internal plan score: {float(score):.0f}/100. Use as an advisory QA ranking signal, not clinical approval."
                )
            except (TypeError, ValueError):
                lines.append(
                    f"- 内部计划评分：{score}。该分数仅用于质量排序提示，不代表临床批准。"
                    if zh else
                    f"- Internal plan score: {score}. Use as an advisory QA ranking signal, not clinical approval."
                )
        ...
        lines.extend([
            "",
            "**临床使用前必须复核**" if zh else "**Required review before clinical use**",
            (
                "- 依据适用的临床指南或已确认的病例方案，复核部位特异性的靶区覆盖和 OAR 耐受限值。"
                if zh else
        ...
        safety = (
            (
                "**安全与质量控制**\n\n"
                "- 复核粒子活度、处方剂量、坐标系、针道和粒子坐标。\n"
                "- 复核 CTV/OAR 掩膜、DVH、热点和 OAR 高剂量区域。\n"
                "- 最终临床限值必须来自检索到的指南证据或已确认的病例方案。\n"
