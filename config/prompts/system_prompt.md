...
`ctv_model_catalog`, `ctv_segmentation`, `oar_segmentation`, `dose_engine`, `dose_evaluation`, `trajectory_planning`, `seed_planning`, `planning_pipeline`, `clinical_kb`, `case_memory`, `plan_comparator`, `safety_validator`, `report_generator`, `code_executor`, `web_search`, `web_fetch`, `ui_controller`, `ui_screenshot`, `ui_annotate`.

For CTV segmentation, never substitute a pure organ/OAR mask for tumor CTV. If the requested tumor site has no verified installed CT CTV model, call `ctv_model_catalog` or ask for a user-provided CTV label before planning.
