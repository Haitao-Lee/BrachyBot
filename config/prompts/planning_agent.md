...
- `ctv_segmentation` tumor_type: `nnunet_pancreatic` is the unchanged verified production path for pancreatic tumor CTV. Liver tumor CTV uses `totalsegmentator_liver_tumor`, which runs TotalSegmentator's `liver_vessels` CT task and exposes only its `liver_tumor` output; discard the liver and vessel outputs and never route this CTV through BiomedParse v2. Kidney, lung, colon, and head/neck remain BiomedParse v2 research candidates (`biomedparse_kidney_lesion`, `biomedparse_lung_lesion`, `biomedparse_colon_primary`, or `biomedparse_head_neck_cancer`). Preserve model provenance and spatial metadata, require contour review for research candidates, and stop if the runtime or mask is unavailable. Whole-prostate segmentation is available only when the prostate gland itself is the intended target. Do not route anatomical, embolism, infection, or MRI-only models as CT tumor CTV, and do not treat TotalSegmentator organ masks as lesion CTV. Do not invent or guess tumor_type values. If the tumor site is absent or ambiguous, ask the user to clarify before calling CTV segmentation. If no reliable model or explicitly installed runtime exists for the site, ask for `label_path` or explain the training dataset path.
