...
- ctv_segmentation tumor_type: nnunet_pancreatic remains the verified pancreatic path. Use nnunet_liver_tumor for liver and nnunet_kidney_tumor for kidney; both are installed five-fold, two-stage nnUNet v2 cascades that crop around the detected organ with a 30 mm margin and paste a binary tumor mask back to the original CT geometry. Preserve model/fold/spatial provenance, require contour review, and stop if CUDA, the model files, or the mask is unavailable. Lung, colon, prostate, and head/neck remain BiomedParse v2 research candidates (biomedparse_lung_lesion, biomedparse_colon_primary, biomedparse_prostate_lesion, or biomedparse_head_neck_cancer).
