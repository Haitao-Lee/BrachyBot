...
- `ctv_segmentation` tumor_type: `nnunet_pancreatic` is the unchanged verified production path for pancreatic tumor CTV. Liver tumor CTV uses `biomedparse_liver_tumor`: BiomedParse v2 automatic text-guided CT candidate segmentation with the official `liver tumors` prompt. Kidney, lung, colon, prostate, and head/neck remain BiomedParse v2 research candidates (`biomedparse_kidney_lesion`, `biomedparse_lung_lesion`, `biomedparse_colon_primary`, `biomedparse_prostate_lesion`, or `biomedparse_head_neck_cancer`). Preserve model provenance and spatial metadata, require contour review for research candidates, and stop if the runtime or mask is unavailable. Do not route anatomical, embolism, infection, or MRI-only models as CT tumor CTV, and do not treat TotalSegmentator organ masks as lesion CTV. Do not invent or guess tumor_type values. If the tumor site is absent or ambiguous, ask the user to clarify before calling CTV segmentation. If no reliable model or explicitly installed runtime exists for the site, ask for `label_path` or explain the training dataset path.
- `oar_segmentation`: use `organ_type: "general"` with no `organ_filter` for a full safety OAR set. For a focused anatomy request without planning, set `organ_filter` to the user-named canonical labels (for example `['liver']`) and retain only those labels in the Data Tree and viewers. A focused OAR result has `oar_is_full=false` and must be replaced by full OAR segmentation before treatment planning.
