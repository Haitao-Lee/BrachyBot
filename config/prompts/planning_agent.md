...
- `ctv_segmentation` tumor_type: `nnunet_pancreatic` is the unchanged verified production path for pancreatic tumor CTV. For liver, kidney, lung, colon, and head/neck, prefer the existing local CT tumor model when available; when its checkpoint is unavailable, the adapter may use the matching explicitly installed BiomedParse v2 research candidate (`biomedparse_liver_tumor`, `biomedparse_kidney_lesion`, `biomedparse_lung_lesion`, `biomedparse_colon_primary`, or `biomedparse_head_neck_cancer`). BiomedParse v2 is text-guided research output, not clinically validated CTV, so preserve its provenance, require contour review, and stop if the runtime or mask is unavailable. Whole-prostate segmentation is available only when the prostate gland itself is the intended target. Do not route anatomical, embolism, infection, or MRI-only models as CT tumor CTV, and do not treat TotalSegmentator organ masks as lesion CTV. Do not invent or guess tumor_type values. If the tumor site is absent or ambiguous, ask the user to clarify before calling CTV segmentation. If no reliable model or explicitly installed BiomedParse runtime exists for the site, ask for `label_path` or explain the training dataset path.
